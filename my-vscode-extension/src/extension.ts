...
class MSCodeViewProvider implements vscode.WebviewViewProvider {
    ...
    private async createCheckpointWithDiff() {
        ...
        const diffs: { [key: string]: { original: string, modified: string } } = {};

        // Vérifier les fichiers ouverts et leurs modifications
        for (const doc of openTextDocuments) {
            ...
            if (fs.existsSync(originalPath)) {
                ...
                if (originalContent !== currentContent) {
                    ...
                    // Sauvegarder les modifications dans le dossier studio
                    const studioDir = path.dirname(studioPath);
                    if (!fs.existsSync(studioDir)) {
                        fs.mkdirSync(studioDir, { recursive: true });
                    }
        ...
        // Créer le checkpoint avec les fichiers modifiés
        const timestamp = new Date().getTime();
        const checkpointDir = path.join(this.workingDirs.changes, `checkpoint_${timestamp}`);
        fs.mkdirSync(checkpointDir, { recursive: true });

        // Copier les fichiers modifiés dans le checkpoint
        changedFiles.forEach(file => {
            const sourcePath = path.join(this.workingDirs!.studio, file);
